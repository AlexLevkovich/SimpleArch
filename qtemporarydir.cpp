...
static QString defaultTemplateName()
{
    ...
    if (baseName.isEmpty()) baseName = QLatin1String("qt_temp");
...
bool QTemporaryDir::removeRecursively(const QString & dirPath)
{
    QDir dir(dirPath);
    if (!dir.exists())
        return true;
    ...
    if (success)
        success = !rmdir(dir.absolutePath().toLocal8Bit().constData());
...
bool QTemporaryDir::remove()
{
    ...
    const bool result = removeRecursively(path());
    if (!result) {
